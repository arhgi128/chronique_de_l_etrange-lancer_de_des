// ═══════════════════════════════════════════════════════════════
// FICHIER : dice.js  (moteur dés 3D — projet teal, modifié)
// Source : http://a.teall.info/dice/ https://emanchado.github.io/3d-die-roller/dice/
// Auteur original  : Anton Natarov (Teal)
// Modifications    : couleurs par valeur (0-9), canvas transparent,
//                    use_true_random=false par défaut,
//                    prepare_rnd via random.org direct (pas $t.rpc)
// Dépendances      : three.js (r73), cannon.js, teal.js
// ═══════════════════════════════════════════════════════════════
...
(function(dice) {
    ...
    var random_storage = [];
    this.use_true_random = false; // désactivé : pas de serveur local
    this.frame_rate = 1 / 60;

    function prepare_rnd(callback) {
        if (!random_storage.length && $t.dice.use_true_random) {
            try {
                // Appel direct à random.org (pas via $t.rpc qui pointe vers "f" local)
                var xhr = new XMLHttpRequest();
                xhr.open('GET', 'https://www.random.org/integers/?num=512&min=0&max=999&col=1&base=10&format=plain&rnd=new', true);
                xhr.timeout = 4000;
                xhr.onload = function() {
                    if (xhr.status === 200) {
                        var nums = xhr.responseText.trim().split('\n').map(function(n){ return parseInt(n)/1000; });
                        random_storage = nums;
                    } else {
    ...
    this.create_dice_materials = function(face_labels, size, margin) {
        // Fond gris anthracite uniforme, chiffre de la couleur de la valeur
        var die_value_colors = {
            '0':  { bg: '#2a2a35', fg: '#c8a050' },  // yin terre - marron doré
            '1':  { bg: '#2a2a35', fg: '#6ab4e8' },  // yang eau - bleu clair
            '2':  { bg: '#2a2a35', fg: '#e03030' },  // yin feu - rouge
            '3':  { bg: '#2a2a35', fg: '#c0c0cc' },  // yang métal - gris clair
            '4':  { bg: '#2a2a35', fg: '#c87840' },  // yin bois - brun orangé
            '5':  { bg: '#2a2a35', fg: '#a07840' },  // yang terre - marron
            '6':  { bg: '#2a2a35', fg: '#4a80d4' },  // yin eau - bleu foncé
            '7':  { bg: '#2a2a35', fg: '#e8c830' },  // yang feu - jaune
            '8':  { bg: '#2a2a35', fg: '#8888a0' },  // yin métal - gris
            '9':  { bg: '#2a2a35', fg: '#4aaa44' },  // yang bois - vert
            ' ':  { bg: '#2a2a35', fg: '#555566' }
        };
        ...
        for (var i = 0; i < face_labels.length; ++i) {
            var lbl = face_labels[i];
            var col = die_value_colors[lbl] || { bg: this.dice_color, fg: this.label_color };
            materials.push(new THREE.MeshPhongMaterial($t.copyto(this.material_options,
                        { map: create_text_texture(lbl, col.fg, col.bg) })));
        }
        return materials;
    }
    ...
    this.desk_color = 0x000000; // sera rendu transparent
    this.use_shadows = true;

//    this.known_types = ['d4', 'd6', 'd8', 'd10', 'd12', 'd20', 'd100'];
// mon ajout
    this.known_types = ['d10', 'd10', 'd10', 'd10', 'd10', 'd10', 'd10'];
// fin de mon ajout
    ...
    this.dice_box = function(container, dimentions) {
        ...
        this.renderer.setClearColor(0x000000, 0); // transparent pour superposition
    ...
    this.dice_box.prototype.reinit = function(container, dimentions) {
        ...
        this.desk = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 2, this.h * 2, 1, 1), 
                new THREE.MeshPhongMaterial({ color: that.desk_color, transparent: true, opacity: 0.0 }));
    ...
    this.dice_box.prototype.bind_mouse = function(container, notation_getter, before_roll, after_roll) {
        var box = this;
        // Tags interactifs à ignorer pour ne pas interférer avec les contrôles UI
        var INTERACTIVE = ['INPUT','BUTTON','SELECT','LABEL','TEXTAREA','A'];
        $t.bind(container, ['mousedown', 'touchstart'], function(ev) {
            // Ignorer si l'événement vient d'un élément interactif ou de ses parents
